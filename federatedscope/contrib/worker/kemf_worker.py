        ...
        if self.check_buffer(self.state, min_received_num, check_eval_result):
            if not check_eval_result:
                ...
                torch.save({'cur_round': self.state, 'model': self.model.state_dict()},
                           os.path.join(self._cfg.outdir, f"server_model_checkpoint.pth"))
                # NOTE(Variant): ---------------------------------------------------------------------------------------
                ...
                if self.state < self.total_round_num:
                    ...
                    for model_idx in range(self.model_num):
                        self.cached_client_models[model_idx].clear()
...
class KEMFClient(EnhanceClient):
    def __init__(self,
                 ID=-1,
                 server_id=None,
                 state=-1,
        ...
        super(KEMFClient, self).__init__(ID, server_id, state, config, data, model, device, strategy, is_unseen_client,
                                         *args, **kwargs)


def call_kemf_worker(method):
    if method == 'FedKEMF' or method == 'fedkemf':
        worker_builder = {'client': KEMFClient, 'server': KEMFServer}
