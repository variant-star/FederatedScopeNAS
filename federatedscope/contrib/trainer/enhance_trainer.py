...
class EnhanceTrainer(GeneralTorchTrainer):
    ...
    def __init__(self,
        ...
        if self.cfg.train.recalibrate_bn:
        ...
        if self._cfg.finetune.before_eval:
            self.replace_hook_in_ft(
                ...
                'on_batch_forward', target_hook_name='_hook_on_batch_forward')
            if self.cfg.finetune.recalibrate_bn:
    ...
    def _run_batch(self, hooks_set):
        ...
        for batch_i in range(
            ...
            if (self.ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE] and
                    self.ctx.cur_epoch_i == getattr(self.ctx, f"num_{self.ctx.cur_mode}_epoch") - 1):  # make changes
    ...
    #     # prepare model and no optimizer
    #     ctx.model.to(ctx.device)
    #
    #     # prepare statistics
    #     ctx.loss_batch_total = CtxVar(0., LIFECYCLE.ROUTINE)
    ...
    def _hook_on_fit_end_for_ft_mode_recalibrate_bn(self, ctx):
        torch.optim.swa_utils.update_bn(ctx.data['server'], ctx.model, ctx.device)
